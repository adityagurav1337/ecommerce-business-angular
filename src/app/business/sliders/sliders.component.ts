...
export class SlidersComponent {
  ...
  load() {
    this.id = 0;
    this.api.get(this.apiurl.sliders + "/" + this.businessid).subscribe((result: any) => {
  ...
  submit(data: any) {
    if (this.id == 0) {
      this.api.post(this.apiurl.sliders, data).subscribe((result: any) => {
        this.api.setMessage({ title: "Success", message: this.api.insert_success_msg, type: "success" });
        this.load();
      })
    }
    else {
      this.api.put(this.apiurl.sliders + "/" + this.id, data).subscribe((result: any) => {
  ...
  edit(id: any) {
    this.id = id;
    this.api.get(this.apiurl.sliders + "/" + this.businessid + "/" + id).subscribe((result: any) => {
  ...
  delete(id: any) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.api.delete(this.apiurl.sliders + "/" + id).subscribe((result: any) => {
